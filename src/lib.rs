...
use log::{info, warn};
use dotenv::dotenv;
use task_queue::queue::{Job, Queue, Message, MessageScope};
...
async fn run_worker(queue: Arc<dyn Queue>) {
    loop {
        let jobs = match queue.pull(MessageScope::GenerateRatings, CONCURRENCY as i32).await {
            Ok(jobs) => jobs,
            Err(e) => {
...
async fn get_event(event: i64, pool: &DB) -> Result<Vec<EventResult>> {
    ...
    for race in races  {
        ...
        event_results.push(EventResult{event_id: event, results: race_result});
    }
    Ok(event_results)
}
